...
const payOrder = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if (!isValidObjectId(orderId)) {
        return res.status(401).json({ message: 'El numero de orden no es valido' })
    }

    const { data } = await axios.get<IPaypal.PaypalOrderStatusResponde>(`${process.env.PAYPAL_ORDERS_URL}/${transactionId}`, {
        headers: {
    ...
    return res.status(201).json({ message: 'Orden pagada correctamente' })
}
